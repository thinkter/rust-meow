...
- protocol v21 of the existing framed protobuf bridge
...
Tauri Rust core (`src-tauri/src/lib.rs`)
  RPC correlation + dialog/opener + sticker preparation
                    |
  4-byte BE length + protobuf Envelope (v20, max 8 MiB)
                    |
Go sidecar
...
2. invoke `hello` and require protocol v21 on both sides;
...
Protocol v20 and attachment, poll, pinned-message, forwarding, editing, sync-integrity, and cache-repair RPCs are wired through the Go backend, fake bridge,
