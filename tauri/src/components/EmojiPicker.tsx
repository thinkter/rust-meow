...
const CATEGORIES: EmojiCategory[] = [
  {
    ...
    entries: [
      ...
      ["😇", "innocent halo angel"],
      ["🥰", "in love hearts smiling adoring"],
      ["😍", "heart eyes love crush"],
      ["🤩", "star struck excited amazed"],
      ["😘", "kiss blow a kiss love"],
      ...
      ["😫", "tired exhausted anguished"],
      ["😩", "weary tired exhausted"],
      ["🥺", "pleading puppy eyes begging"],
      ["😢", "crying sad tear"],
      ["😭", "sobbing crying loud bawling"],
      ["😤", "huffing frustrated proud steam"],
      ["😠", "angry mad"],
      ["😡", "furious rage angry red"],
      ["🤬", "cursing swearing angry symbols"],
      ["🤯", "mind blown shocked exploding head"],
      ...
      ["😓", "sweat downcast tired"],
      ["🤗", "hug hugging warm"],
      ["🤔", "thinking hmm pondering"],
      ["🤭", "giggle oops covering mouth"],
      ["🤫", "shush quiet secret"],
...
/** Flat emoji → keywords lookup, built once at module load. */
const KEYWORDS: Record<string, string> = CATEGORIES.reduce<Record<string, string>>((map, category) => {
  for (const [emoji, keywords] of category.entries) map[emoji] = keywords;
  return map;
}, {});
...
export function EmojiPicker(props: {
  ...
  const filteredEmojis = createMemo(() => {
    ...
    for (const category of CATEGORIES) {
      for (const [emoji, keywords] of category.entries) {
